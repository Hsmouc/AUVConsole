...
def fetch_values():
    global AUV_yaw,water_temperature
    try:
        s = ser.readlines()
    except:
        pass
...
def send_values(root,GUI):
    ...
    isZeroSet = 0
    ser.write(package)
    root.after(200,lambda:send_values(root,GUI))
...
class consoleGUI:
	...
	self.canvas = Canvas(width=180,height=180,bg='white',highlightthickness=0)
	self.canvas.place(x=50,y=420) 
